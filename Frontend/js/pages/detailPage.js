/**
 * HomeOfEmlak — Detail Page
 */
import { getPropertyById, getSimilarProperties } from '../data.js';
...
export function renderDetailPage(container, params) {
        ...
        <div class="detail-page__content">
          ...
          <div>
            <div class="owner-card">
              ...
              <a href="tel:${property.owner.phone}" class="btn btn--primary" style="width:100%;margin-bottom:var(--space-3)">
                📞 ${property.owner.phone}
              </a>
              <a href="https://wa.me/${property.owner.phone.replace(/\D/g, '')}" target="_blank" class="btn btn--outline" style="width:100%;margin-bottom:var(--space-3);color:#25D366;border-color:#25D366">
                💬 WhatsApp
              </a>
              <button class="btn btn--ghost" style="width:100%;margin-bottom:var(--space-3)" id="msg-btn">
                ✉️ Mesaj Gönder
              </button>
              <button class="btn btn--ghost" style="width:100%" id="detail-fav-btn">
                ${fav ? '❤️ Favorilerde' : '🤍 Favorilere Ekle'}
              </button>
            </div>
  ...
  container.querySelectorAll('#gallery img').forEach(img => {
    img.addEventListener('click', () => {
      ...
      openLightbox(property.images, idx);
    });
  });
  ...
  try {
    if (window.L) {
      const map = L.map('detail-map').setView([property.location.lat, property.location.lng], 14);
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        attribution: '© OpenStreetMap'
      }).addTo(map);
      L.marker([property.location.lat, property.location.lng]).addTo(map)
